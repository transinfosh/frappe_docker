...
平台以独立 Frappe 应用交付，应用名为 `doc`，中文名为“文档”。它面向两类用户：
...
`doc` app 采用“Frappe 平台内核 + Vue SPA 前端”的架构。
...
| `latest_revision` | Link | 最近一次页面修订 |
| `published_snapshot` | Link | 当前有效页面快照 |

### 5.6 DOC0031 文档页面修订

表示页面草稿的编辑历史记录，用于查看历史内容、对比差异和恢复草稿。它不同于 `DOC0020 文档版本`，也不同于 `DOC0050 文档页面快照`：

- `DOC0020 文档版本` 是产品级文档空间，例如 `v1`、`v2`。
- `DOC0031 文档页面修订` 是某个页面草稿的编辑历史。
- `DOC0050 文档页面快照` 是对外发布后的不可变线上内容。
...
| `project` | Link | 所属文档项目 |
| `doc_version` | Link | 所属文档版本 |
| `revision_no` | Int | 页面内递增修订号 |
| `title` | Data | 修订时页面标题 |
| `content_markdown` | Long Text | 修订时 Markdown 内容 |
| `summary` | Small Text | 修订说明或自动摘要 |
| `change_type` | Select | `manual_save`、`auto_save`、`restore`、`publish` |
| `created_by` | Link | 修改人 |
| `created_at` | Datetime | 修改时间 |

修订记录建议在用户显式保存时创建；自动保存可只更新当前草稿，或按时间窗口合并生成修订，避免产生过多碎片。发布时应确保当前发布内容对应一个修订记录，方便从发布快照追溯到当时的草稿来源。
...
| 概念 | DocType | 解决的问题 | 示例 |
| --- | --- | --- | --- |
| 产品文档版本 | `DOC0020 文档版本` | 同一项目下多套对外文档空间 | `payment/v1`、`payment/v2` |
| 页面修订版本 | `DOC0031 文档页面修订` | 某个页面草稿的编辑历史、diff 和恢复 | “创建订单”第 12 次保存 |
| 发布页面快照 | `DOC0050 文档页面快照` | 对外发布后的不可变线上内容 | 2026-05-17 发布的线上页面 |

产品文档版本决定 URL 和文档树范围；页面修订版本服务内部编辑审计；发布页面快照服务外部阅读和回滚。一个页面可以有多次修订，其中部分修订会被发布为页面快照。

发布采用“草稿与线上快照隔离”模型。

内部用户编辑 `DOC0030 文档页面.content_markdown` 和 `DOC0040 图表块.source_content`。用户显式保存页面时生成 `DOC0031 文档页面修订`；点击发布时系统生成 `DOC0080 文档发布记录`，并基于当前页面修订生成对应的页面快照和图表快照。
...
恢复草稿和回滚发布是两个不同动作。恢复草稿基于 `DOC0031 文档页面修订`，只影响内部草稿内容，不改变外部已发布页面。回滚发布基于 `DOC0050 文档页面快照`，系统生成新的 `rollback` 发布记录，并将目标页面的当前有效快照切换到回滚结果。这样可以保留完整审计链。
...
| 页面修订 | 显式保存生成修订、修订号递增、基于修订恢复草稿 | 后端 / 接口 |
